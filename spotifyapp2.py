...
st.header('Spotify Analytics App - Version MVP')

search_choices = ['Song/Track', 'Artist', 'Album']

st.sidebar.write('Sidebar')

search_selected = st.sidebar.selectbox("Choose what you are looking for",search_choices)

search_keyword = st.text_input(search_selected + " - (Look up a keyword)")
...
if selected_track is not None and len(tracks) > 0:
    ...
    if track_id is not None:
        ...
        if selected_track_choice == 'Song Features':
            ...
            token = songrecommendations.get_token(spotify_client_id, spotify_client_secret)
